...
from KmerDistanceEvolution.ParameterEstimation.Tree import reconstruct_tree_CoalescentJCLS, reconstruct_tree_CoalescentJCNJ, reconstruct_tree_NJ, reconstruct_tree_raxml
...
def main(argv):
    ...
    for row,b in enumerate([(row+1)*(b_max)/nr_rows for row in range(nr_rows)]):
        for col,a in enumerate([(col+1)*(a_max)/nr_cols for col in range(nr_cols)]):
            ...
            n = len(specieses)
            species_names = [species.name for species in specieses]
            genes = [GeneLineage(name='s{:d}'.format(i)) for i,_ in enumerate(range(len(specieses)))]
            ...
            for sim_it in range(nr_sims):
                    ...
                    for distance_formula in ['dstar', 'ARS2015']:
                        for k_i in k:
                            if distance_formula == 'ARS2015':
                                ...
                                dm = kmer_distance_matrix(sequences, k_i, dstar_kmer_distance, grouping=base_embedding)
                                #print(dm)
                            else:
                                raise Exception
                ...
                for distance_formula in ['dstar', 'ARS2015']:
                    for k_i in k:
                        ...
                        session.add(kdm)
                jc_dm = kmer_distance_matrix(concatenated_sequences.values(), 1, aligned_kmer_distance, alignment_fn=stretcher_alignment, grouping=base_embedding)
                #print(jc_dm)
                ...
                with TemporaryDirectory() as tmpdir:
                    t0 = time.clock()
                    xtreehat = reconstruct_tree_raxml(concatenated_sequences.values(), base_embedding, tmpdir)
                    t1 = time.clock()
    ...
    for tree_estimate_set in tree_estimate_sets:
        ...
        for huel_sim in tree_estimate_set.simulation_set.huelsenbeck_simulations:
            ...
            if method == 'CoalescentJCLS':
                xtreehat = reconstruct_tree_CoalescentJCLS(kmer_distance_matrices)
            elif method == 'CoalescentJCNJ':
                xtreehat = reconstruct_tree_CoalescentJCNJ(kmer_distance_matrices)
            elif method == 'JCNJ':
                #for k,dm in kmer_distance_matrices.items():
                #    print dm
                adjusted_distance_matrices = dict((k,JCadjustment(kmer_distance_matrix,k)) for k,kmer_distance_matrix in kmer_distance_matrices.items()) 
                #for k,dm in adjusted_distance_matrices.items():
                #    print dm
                xtreehat = reconstruct_tree_NJ(adjusted_distance_matrices)
            elif method == 'dstarNJ':
                #for _,dm in kmer_distance_matrices.items():
                #    print dm
                xtreehat = reconstruct_tree_NJ(kmer_distance_matrices)
            elif method == 'concatdJCNJ':
                adjusted_distance_matrices = {1:JCadjustment(kmer_distance_matrices[1],1)}
                xtreehat = reconstruct_tree_NJ(adjusted_distance_matrices)
            else:
                raise(Exception)
            t1 = time.clock()
